defmodule QuizTest do
  ...
  describe "a quiz that always adds one or two" do
    setup ~w(quiz_always_adds_one_and_two)a
  ...
  defp correct_answer(quiz), do: answer_question(quiz, "3")
  defp incorrect_answer(quiz), do: answer_question(quiz, "wrong")
  ...
  defp refute_more_question(quiz) do
    assert is_nil(quiz)
    quiz
  end
end
