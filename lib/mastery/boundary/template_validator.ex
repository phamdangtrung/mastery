defmodule Mastery.Boundary.TemplateValidator do
  ...
  def errors(fields) when is_list(fields) do
    ...
    |> optional(fields, :instructions, &validate_instruction/1)
  ...
  def validate_instruction(instruction) when is_binary(instruction), do: :ok
  def validate_instruction(_instruction), do: {:error, "must be a binary"}
  ...
  def validate_generator(_generators), do: {:error, "must be a string to list or function pair"}
